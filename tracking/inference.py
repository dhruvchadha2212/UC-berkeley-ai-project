...
class ParticleFilter(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance is None:
            ...
            for particlePos in self.particles:
                weights[particlePos] += emissionModel[util.manhattanDistance(pacmanPosition, particlePos)]
            weights.normalize()
    ...
    def elapseTime(self, gameState):
        ...
        newParticles = []
        for particlePos in self.particles:
            newPosDistOfParticle = self.getPositionDistribution(self.setGhostPosition(gameState, particlePos))
            newParticles.append(util.sample(newPosDistOfParticle))
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        twoGhostsPositions = list(itertools.product(ghostPositions, repeat = self.numGhosts))
        random.shuffle(twoGhostsPositions)
        self.particles = twoGhostsPositions[:self.numParticles]
        print "after", len(self.particles)
    ...
    def observeState(self, gameState):
        ...
        weights = util.Counter()
        for particlePosTuple in self.particles:
            prod = 1
            for i in range(self.numGhosts):
                if noisyDistances[i] != None:
                    prod *= emissionModels[i][util.manhattanDistance(pacmanPosition, particlePosTuple[i])]
            weights[particlePosTuple] += prod
        weights.normalize()
        ...
        which you are computing the new position distribution.
{
    ...
    def getBeliefDistribution(self):
        ...
        self.beliefs = util.Counter()
        for particlePosTuple in self.particles:
            self.beliefs[particlePosTuple] += 1;
        self.beliefs.normalize()
        return self.beliefs
        # util.raiseNotDefined()
