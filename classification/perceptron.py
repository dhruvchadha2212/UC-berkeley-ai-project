        ...
        for iteration in range(self.max_iterations):
            ...
            for i in range(len(trainingData)):
                ...
                if not (trainingLabels[i] == lab.argMax()):
                    ...
                    # self.weights[trainingLabels[i]].__add__ (trainingData[i])
...
def findHighWeightFeatures(self, label):
    ...
    sorted_d = sorted (self.weights[label].items (), key=operator.itemgetter (1), reverse=True)
    count=0
    for key in sorted_d:
        if count>=150:
            break
        featuresWeights.append(key[0])
        count=count+1
            # for key in sorted_d:
            #     # if len(featuresWeights)>150:
            #     #     br
            #     if self.weights[label][key]>0:
            #       featuresWeights.append(key)
            # # print(self.weights[label][16,9])
            # # k1[16,9]=1
            # # featuresWeights.append(k1)
            # "*** YOUR CODE HERE ***"
